...
async function carregarStatusZapi() {
    const loading = document.getElementById('whatsapp-loading');
    const conteudo = document.getElementById('whatsapp-conteudo');
    if (!loading || !conteudo) return;

    try {
        const res = await fetch('/api/config/zapi', {
            headers: { Authorization: `Bearer ${token}` }
        });
        const d = await res.json();
        ...
        // Badge status
        const badgeStatus = document.getElementById('zapi-badge-status');
        const configurado = d.configurado;
        badgeStatus.textContent = configurado ? '✅ Conectado' : '⚪ Não configurado';
        badgeStatus.style.cssText = `display:inline-block;padding:3px 10px;border-radius:20px;font-size:12px;font-weight:700;letter-spacing:0.5px;background:${configurado ? '#d1fae5;color:#065f46' : '#f1f5f9;color:#475569'};`;

        // Badge chave
        const badgeChave = document.getElementById('zapi-badge-chave');
        badgeChave.textContent = configurado ? '🔑 Credenciais salvas' : '🔓 Sem credenciais';
        badgeChave.style.cssText = `display:inline-block;padding:3px 10px;border-radius:20px;font-size:12px;font-weight:700;letter-spacing:0.5px;background:${configurado ? '#dbeafe;color:#1e40af' : '#f1f5f9;color:#475569'};`;

        // Mostrar botão remover e testar só se configurado
        document.getElementById('btnRemoverZapi').style.display = configurado ? 'inline-block' : 'none';
        document.getElementById('btnTestarZapi').style.display = configurado ? 'inline-block' : 'none';

        // Placeholders
        const ph = configurado ? '••••••••••••••• (configurado — cole para substituir)' : '';
        document.getElementById('zapiInstanceIdInput').placeholder = ph || 'Ex: 3D5F2A1B...';
        document.getElementById('zapiTokenInput').placeholder = ph || 'Ex: A1B2C3D4...';
        document.getElementById('zapiClientTokenInput').placeholder = ph || 'Ex: Fc57e...';

        // Preencher dias de follow-up com valores salvos
        if (d.dias_lead)     document.getElementById('zapiDiasLead').value     = d.dias_lead;
        if (d.dias_triagem)  document.getElementById('zapiDiasTriagem').value  = d.dias_triagem;
        if (d.dias_proposta) document.getElementById('zapiDiasProposta').value = d.dias_proposta;

    } catch {
        if (loading) loading.textContent = 'Erro ao carregar configuração de WhatsApp.';
    }
}

function toggleVerZapiInstanceId() {
    const i = document.getElementById('zapiInstanceIdInput');
    i.type = i.type === 'password' ? 'text' : 'password';
}

function toggleVerZapiToken() {
    const i = document.getElementById('zapiTokenInput');
    i.type = i.type === 'password' ? 'text' : 'password';
}

function toggleVerZapiClientToken() {
    const i = document.getElementById('zapiClientTokenInput');
    i.type = i.type === 'password' ? 'text' : 'password';
}

async function salvarConfigZapi() {
    const instanceId   = document.getElementById('zapiInstanceIdInput').value.trim();
    const token_zapi   = document.getElementById('zapiTokenInput').value.trim();
    const clientToken  = document.getElementById('zapiClientTokenInput').value.trim();
    const diasLead     = parseInt(document.getElementById('zapiDiasLead').value) || 3;
    const diasTriagem  = parseInt(document.getElementById('zapiDiasTriagem').value) || 5;
    const diasProposta = parseInt(document.getElementById('zapiDiasProposta').value) || 4;

    // Permite salvar só os dias mesmo sem mudar credenciais
    const btn = document.getElementById('btnSalvarZapi');
    btn.disabled = true;
    btn.textContent = 'Salvando…';

    try {
        const body = { dias_lead: diasLead, dias_triagem: diasTriagem, dias_proposta: diasProposta };
        if (instanceId)  body.instance_id   = instanceId;
        if (token_zapi)  body.token         = token_zapi;
        ...
        // Limpar campos de credencial após salvar
        document.getElementById('zapiInstanceIdInput').value = '';
        document.getElementById('zapiTokenInput').value = '';
        document.getElementById('zapiClientTokenInput').value = '';

        mostrarMsgZapi('✅ Configuração salva! O WhatsApp automático está ativo no CRM.', '#065f46');
        carregarStatusZapi();
    } catch { alert('Erro de conexão.'); }
...
async function testarZapi() {
    const btn = document.getElementById('btnTestarZapi');
    btn.disabled = true;
    btn.textContent = '⏳ Testando…';

    try {
        const res = await fetch('/api/config/zapi/testar', {
            method: 'POST',
            headers: { Authorization: `Bearer ${token}` }
        });
        const d = await res.json();
        if (d.ok) {
            mostrarMsgZapi('✅ Conexão Z-API funcionando! Instância ativa.', '#065f46');
        } else {
            mostrarMsgZapi('❌ ' + (d.erro || 'Conexão falhou. Verifique as credenciais.'), '#991b1b');
        }
    } catch { mostrarMsgZapi('❌ Erro de conexão ao testar.', '#991b1b'); }
    finally { btn.disabled = false; btn.textContent = '📱 Testar Conexão'; }
}

async function removerConfigZapi() {
    if (!confirm('Remover configuração do WhatsApp? As mensagens automáticas serão desativadas.')) return;
    ...
    try {
        ...
        if (res.ok) {
            mostrarMsgZapi('Configuração removida.', '#64748b');
            carregarStatusZapi();
        }
    } catch { alert('Erro de conexão.'); }
}

function mostrarMsgZapi(texto, cor) {
    const el = document.getElementById('zapi-msg');
    el.textContent = texto;
    el.style.color = cor;
    el.style.display = 'block';
